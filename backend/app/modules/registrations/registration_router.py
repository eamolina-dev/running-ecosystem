...
from app.modules.registrations.registration_schema import RegistrationCreate, RegistrationUpdate, RegistrationRead
from app.modules.registrations import registration_service
...
def update_registration(reg_id: int, reg_in: RegistrationUpdate, db: Session = Depends(get_db)):
    return registration_service.update(db, reg_id, reg_in)


@router.delete("/{reg_id}")
def delete_registration(reg_id: int, db: Session = Depends(get_db)):
    return registration_service.delete(db, reg_id)
